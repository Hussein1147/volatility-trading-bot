...
async def test_data_sources():
    ...
    print(f"\n{Fore.YELLOW}3. Testing Alpaca Options Data (Feb 2024+)...")
    try:
        # Get options chain for recent date
        options_chain = await alpaca_fetcher.get_options_chain(symbol, recent_date, 30, 45)
        
        if options_chain:
            print(f"{Fore.GREEN}✓ Alpaca returned {len(options_chain)} options")
            ...
            if options_with_greeks:
                ...
                print(f"{Fore.GREEN}✓ Alpaca Greeks available:")
                print(f"{Fore.GREEN}  - Sample: {sample['symbol']}")
                print(f"{Fore.GREEN}  - Delta: {sample['delta']:.3f}")
                print(f"{Fore.GREEN}  - Gamma: {sample.get('gamma', 'N/A')}")
                print(f"{Fore.GREEN}  - Theta: {sample.get('theta', 'N/A')}")
                print(f"{Fore.GREEN}  - Vega: {sample.get('vega', 'N/A')}")
            else:
                print(f"{Fore.YELLOW}  - No Greeks found in Alpaca data")
            
            # Show data source used
            if any('polygon' in str(opt.get('source', '')).lower() for opt in options_chain):
                print(f"{Fore.CYAN}  - Data source: Polygon (via integration)")
            elif all(opt.get('delta') is None for opt in options_chain):
                print(f"{Fore.YELLOW}  - Data source: Simulated (no real Greeks)")
            else:
                print(f"{Fore.GREEN}  - Data source: Alpaca (real data)")
        else:
            print(f"{Fore.RED}✗ No options data returned")
            
    except Exception as e:
        print(f"{Fore.RED}✗ Alpaca error: {e}")
    
    # 4. Test Integrated Data Fetcher
    print(f"\n{Fore.YELLOW}4. Testing Integrated Data Fetcher...")
    
    # Test historical date (should use Polygon or simulation)
    print(f"\n{Fore.CYAN}Historical date ({historical_date.strftime('%Y-%m-%d')}):")
    hist_chain = await alpaca_fetcher.get_options_chain(symbol, historical_date, 30, 45)
    if hist_chain:
        print(f"{Fore.GREEN}✓ Got {len(hist_chain)} options")
    ...
    print(f"\n{Fore.CYAN}Recent date ({recent_date.strftime('%Y-%m-%d')}):")
    recent_chain = await alpaca_fetcher.get_options_chain(symbol, recent_date, 30, 45)
    if recent_chain:
        print(f"{Fore.GREEN}✓ Got {len(recent_chain)} options")
    ...
    print(f"\n{Fore.CYAN}Testing IV Rank integration:")
    vol_data = await alpaca_fetcher.get_historical_volatility_data(symbol, recent_date, 365)
    if vol_data:
        print(f"{Fore.GREEN}✓ IV Rank: {vol_data['iv_rank']:.1f}")
    ...
    if data_sources:
        print(f"{Fore.GREEN}✓ Active real data sources:")
        for source in data_sources:
            print(f"  - {source}")
    else:
        print(f"{Fore.RED}✗ No real data sources active - using simulation only")
